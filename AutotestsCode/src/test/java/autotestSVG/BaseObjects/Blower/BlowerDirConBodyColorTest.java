...
public class BlowerDirConBodyColorTest {
    ...
    public static void oneTimeSetup() {
        ...
        InsatWebUITools tester = new InsatWebUITools();
        tester.action.wait(driver, "//*[@data-control-id = '119437']");
        tester.set.colorButton(driver, "//*[@data-control-id = '119437']");
    }
    ...
    @Description("Сравнение скриншота актуальной страницы с эталоном (Воздуходувка профиль, прямая конвертация, проверка динамизации параметра [Цвет корпуса])")
    ...
    public void vozduhoduvkaProfil(){
        ...
        tester.check.screenshotBasedTest(driver, PATH, "vozduhoduvkaProfil_bodyColor", XPath);
    }

    @Description("Сравнение скриншота актуальной страницы с эталоном (Воздуходувка верхний выход, прямая конвертация, проверка динамизации параметра [Цвет корпуса])")
    @Links(value = {@Link(name = "TestProject", url = TPROJECT_LINK), @Link(name = "Rizzoma", url = RIZZOMA_LINK)})
    @Epic(value = "Библиотека [BaseObjects]")
    @Feature(value = "Воздуходувки")
    ...
    public void vozduhoduvkaVerhniyVihod(){
        ...
        local.waitSVG(driver, XPath);
        tester.check.screenshotBasedTest(driver, PATH, "vozduhoduvkaVerhniyVihod_bodyColor", XPath);
    }

    @Description("Сравнение скриншота актуальной страницы с эталоном (Воздуходувка боковой выход, прямая конвертация, проверка динамизации параметра [Цвет корпуса])")
    @Links(value = {@Link(name = "TestProject", url = TPROJECT_LINK), @Link(name = "Rizzoma", url = RIZZOMA_LINK)})
    @Epic(value = "Библиотека [BaseObjects]")
    @Feature(value = "Воздуходувки")
    ...
    public void vozduhoduvkaBokovoyVihod(){
        ...
        local.waitSVG(driver, XPath);
        tester.check.screenshotBasedTest(driver, PATH, "vozduhoduvkaBokovoyVihod_bodyColor", XPath);
    }
}
