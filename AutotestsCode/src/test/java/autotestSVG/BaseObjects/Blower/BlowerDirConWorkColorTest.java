...
public class BlowerDirConWorkColorTest {
    ...
    public static void oneTimeSetup() {
        ...
        tester.action.wait(driver, "//*[@data-control-id = '119284']");
        tester.set.colorButton(driver, "//*[@data-control-id = '119284']");
    }
    ...
    @Description("Сравнение скриншота актуальной страницы с эталоном (Воздуходувка профиль, прямая конвертация, проверка динамизации параметра [Цвет работы])")
    ...
    public void vozduhoduvkaProfil(){
        ...
        tester.check.screenshotBasedTest(driver, PATH, "vozduhoduvkaProfil_workColor", XPath);
    }

    @Description("Сравнение скриншота актуальной страницы с эталоном (Воздуходувка верхний выход, прямая конвертация, проверка динамизации параметра [Цвет работы])")
    @Links(value = {@Link(name = "TestProject", url = TPROJECT_LINK), @Link(name = "Rizzoma", url = RIZZOMA_LINK)})
    @Epic(value = "Библиотека [BaseObjects]")
    @Feature(value = "Воздуходувки")
    ...
    public void vozduhoduvkaVerhniyVihod(){
        ...
        local.waitSVG(driver, XPath);
        tester.check.screenshotBasedTest(driver, PATH, "vozduhoduvkaVerhniyVihod_workColor", XPath);
    }

    @Description("Сравнение скриншота актуальной страницы с эталоном (Воздуходувка боковой выход, прямая конвертация, проверка динамизации параметра [Цвет работы])")
    @Links(value = {@Link(name = "TestProject", url = TPROJECT_LINK), @Link(name = "Rizzoma", url = RIZZOMA_LINK)})
    @Epic(value = "Библиотека [BaseObjects]")
    @Feature(value = "Воздуходувки")
    ...
    public void vozduhoduvkaBokovoyVihod(){
        ...
        local.waitSVG(driver, XPath);
        tester.check.screenshotBasedTest(driver, PATH, "vozduhoduvkaBokovoyVihod_workColor", XPath);
    }
}
