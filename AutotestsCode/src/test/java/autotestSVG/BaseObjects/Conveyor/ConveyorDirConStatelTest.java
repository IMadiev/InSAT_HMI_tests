...
public class ConveyorDirConStatelTest {
    ...
    @Description("Сравнение скриншота актуальной страницы с эталоном (Шнек, прямая конвертация, проверка динамизации параметра [Состояние шнека] шнеков)")
    ...
    public void screw(){
        ...
        tester.check.screenshotBasedTest(driver, PATH, "screw_stateStarted", XPath);
        tester.set.comboBox(driver, "//*[@data-control-id = '112655']", "Stopped");
        local.waitSVG(driver, XPath);
        tester.check.screenshotBasedTest(driver, PATH, "screw_stateStopped", XPath);
    }

    @Description("Сравнение скриншота актуальной страницы с эталоном (Шнек изометрия, прямая конвертация, проверка динамизации параметра [Состояние шнека] шнеков)")
    @Links(value = {@Link(name = "TestProject", url = TPROJECT_LINK), @Link(name = "Rizzoma", url = RIZZOMA_LINK)})
    @Epic(value = "Библиотека [BaseObjects]")
    @Feature(value = "Конвейеры и шнеки")
    ...
    public void screwIsometriya(){
        ...
        tester.set.comboBox(driver, "//*[@data-control-id = '112655']", "Started");
        local.waitSVG(driver, XPath);
        tester.check.screenshotBasedTest(driver, PATH, "screwIsometriya_stateStarted", XPath);
        tester.set.comboBox(driver, "//*[@data-control-id = '112655']", "Stopped");
        local.waitSVG(driver, XPath);
        tester.check.screenshotBasedTest(driver, PATH, "screwIsometriya_stateStopped", XPath);
    }
}
